...
- SQL防注入
  - 限制输入只允许数字加字母的组合，并限制长度
  - 所有传进来的参数都会通过mysqlpp转移，不会被mysql引擎执行
- 用户密码使用bcrypt hash，并加入每个用户都不同的随机盐，即使被拖库攻击者也无法一次性通过彩虹表猜出用户的弱密码，同时bcrypt也限制了攻击者遍历一次彩虹表需要几年的时间
